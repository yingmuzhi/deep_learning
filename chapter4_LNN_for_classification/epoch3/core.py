...
def show_images(imgs, num_rows, num_cols, titles=None, scale=1.5):
    """
    intro:
        plot images. figsize's unit is inches.
    args:
        :param ndarray/torch.Tensor imgs: a list of images want to show.
        :param int num_rows: how many rows wanna pint. ---. row related with y.
        :param int num_cols: how many cols wanna pint. | |. col related with x. figsize's unit is inches.
        :param list titles: labels.
...
class ModelModule(torch.nn.Module, HyperParameters):
    ...
    def forward(self, X):
        ...
        args:
            :param torch.Tensor X: X is just the `signal/inputs`, and it will return the `pred` you wanna get since it will make a comparison with `target/label`.
        """
    ...
    def plot(self, key, value, train):
        ...
        if train:
            x = self.trainer.train_batch_idx / self.trainer.num_train_batches   # less than 1, [0, 1]
            n = self.trainer.num_train_batches / self.plot_train_per_epoch      # per epoch, plot how many points, if plot_train_per_epoch==2, then when x == [0.5, 1], points will be ploted.
        else:
    ...
    def training_step(self, batch):
        ...
        """
        l = self.loss(self(*batch[:-1]), batch[-1]) # using `*` just because the batch[:-1] will return a list \ # using `forward(X)` to get y_hat, then calculate loss = loss(y_hat, y)
...
class TrainerModule(HyperParameters):
    ...
    def prepare_model(self, model):
        """
        intro:
            set `trianer.model` and `model.trainer`, `model.board.xlim`.
        """
    ...
    def prepare_data(self, data):
        ...
        intro:
            set trainer attributes. 
            set `trainer.train_dataloader`  using `data.train_dataloader()`.
            set `trainer.val_dataloader`    using `data.val_dataloader()`.
            set `trainer.num_train_batches` using `len(trainer.train_dataloader)`.
            set `trainer.num_val_batches`   using `len(trainer.val_dataloader)`.
        """
    ...
    def fit(self, model, data):
        ...
        """
        self.prepare_data(data)     # regist `DataModule`
        self.prepare_model(model)   # regist `ModelModule`
        self.optim = model.configure_optimizers()   # regist `ModelModule`
        self.epoch = 0
